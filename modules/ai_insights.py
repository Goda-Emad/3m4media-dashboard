...
def show_ai_insights(df, lang="en", theme="dark"):
    ...
    success_bg = "rgba(0,180,180,0.08)" if theme == "dark" else "rgba(0,120,120,0.07)"
    ...
    recs = [
        ...
        {
            "icon": "⚠️",
            "title": f"Underperforming Platform: {worst_platform}",
            "desc":  f"Consider reducing budget allocation here and reallocating to {best_platform}",
            "color": "#FFB347"
        },
    ]
    ...
    with col_chart:
        ...
        pred_rois   = [
            last_roi * (1 + growth_rate),
            last_roi * (1 + growth_rate) ** 2,
            last_roi * (1 + growth_rate) ** 3
        ]
    ...
    for col, (icon, label, val) in zip([col1,col2,col3,col4,col5,col6], stats):
        with col:
            st.markdown(f"""
            <div style='background:{card_bg}; border:1px solid {border};
